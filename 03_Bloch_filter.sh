...
#for PCA, EEMS, IBD 
vcftools --vcf ${workdir}/03_vcf/blochmannia_final.g.vcf --keep keeplist_pca.txt --max-missing 1.0   --minDP 10  --min-alleles 2 --max-alleles 2 --mac 2  --max-maf 0.49  --recode --recode-INFO-all --out ${workdir}/04_filtered_vcf/blochmannia_final

# run vcftools with SNP and invariant site output, 20% max missing data, no indels 
#for Phylogeny 
vcftools --vcf ${workdir}/03_vcf/blochmannia_final.g.vcf --max-missing 0.8 --minGQ 20 --minDP 10  --max-alleles 2  --max-maf 0.49 --remove-indels --recode --recode-INFO-all --out ${workdir}/05_phylo_vcf/blochmannia_final_phylo
