...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	options(scipen=999)
	project_directory <- "/lustre/scratch/sboyane/camponotus"
	directory_name <- "11_genotype_scripts"
	reference_genome_location <- "/lustre/work/sboyane/ref/camp_sp_genome_filtered.fasta"
	cluster <- "quanah"
	output_name <- "camp_genotype"
	popmap <- "/lustre/scratch/sboyane/camponotus/popmap.txt"
	individuals <- read.table(popmap, sep="\t")
	faidx <- read.table("/lustre/work/sboyane/ref/camp_sp_genome_filtered.fasta.fai", stringsAsFactors=F)
